...
def main():
    ...
    if router_ip:
        ...
        log_warn("Failed to detect router IP. Falling back to default if needed.")
    ...
    if user_access_level == "admin":
        ...
        service_filter = service_filter_input.split(",") if service_filter_input else None
        ...
        monitor_mode_dongle = input("🔌 Do you have a monitor mode dongle (Y/N)? ").strip().lower()

        if monitor_mode_dongle == 'y' and CONFIG.get('monitor_interface'):
            print("📡 Monitor mode dongle detected. Starting Wi-Fi scan...")

            monitor = WiFiMonitor()
            monitor.scan()

        else:
            print("❌ No monitor mode dongle found. Proceeding with basic scan...")
